...
#Case Study 1: Heights Data from dslab
...
We start by defining the outcome and predictor. 
...
Not surprisingly, our accuracy is about 53%. We are guessing!
...
The accuracy goes up from 0.53 to about 0.79:
...
which is much higher than 0.53. The cutoff resulting in this accuracy is:
...
We see that it is a little less than the accuracy observed for the training set and better than guessing. And by testing on a dataset that we did not train on, we know our result is not due to cherry-picking a good result.
...
To define sensitivity and specificity, we need a binary outcome. When the outcomes are categorical, we can define these terms for a specific category. In the this example, we can ask for the specificity in the case of correctly predicting _male_ as opposed to _female_. Once we specify a category of interest, then we can talk about positive outcomes, $Y=1$, and negative outcomes, $Y=0$.
...
A cutoff of `best_cutoff` makes more sense than 64. Furthermore, it balances the specificity and sensitivity of our confusion matrix:
...
As an example, let's provide a prediction for a patient whose tumor was 4mm.  What is the conditional probability of dying if your tumor was 4mm? In our dataset we can estimate this by rounding to the nearest mm and computing:
...
This plot suggests that the conditional probability generally increases with $x$ and, at least in some parts, appears to be a linear function of $x$. Let's assume that this is the case for now and use the following model:
...
betas <- train_set %>% 
  ...
  reframe(tidy(lm(y ~ thickness, data = .))) %>%
  .$estimate
```
...
## Logistics regression and Naive Bayes Models with DHS Data
Let consider fitting logistics regression and naive Bayes model to Ghana DHS 2022 data for predicting a underweight of children.
The outcome variable $Y$ is binary, that is 1 for underweight and 0 normal. 

```{r}
#load the data
GHDHS22<-read.csv("GHDHS22.csv")
GHDHS22<-GHDHS22%>%mutate(wealth_cat=case_when(wealth_status=="poorest"~ "0", wealth_status=="poorer"~ "1", wealth_status=="middle"~"2", wealth_status=="richer"~ "3", TRUE~ "4"))
GHDHS22$residence<-as.factor(GHDHS22$residence)
GHDHS22$wealth_cat<-as.factor(GHDHS22$wealth_cat)
GHDHS22$anemia2<-as.factor(GHDHS22$anemia2)
GHDHS22$mother_edu2<-as.factor(GHDHS22$mother_edu2)
GHDHS22$sex<-as.factor(GHDHS22$sex)
GHDHS22$underweight1<-as.factor(GHDHS22$underweight1)
head(GHDHS22)
```

We will consider the binary outcome variable, *underweight1* and covariates or predictors: `wealth_status`,`mother_edu2`, `anemia2` and `residence`.
...
#library(caret)
set.seed(1011)
train_ind <- createDataPartition(GHDHS22$underweight1, times = 1, p = 0.7, list = FALSE)

train <- GHDHS22[train_ind, ]
test <- GHDHS22[-train_ind, ]
...
log_fit<- glm(underweight1 ~ anemia2+mother_edu2+wealth_cat+sex, data=train, family = "binomial")
```

We can obtain predictions using the predict function:

```{r}
p_hat_logit1 <- predict(log_fit, newdata = test, type="response")
head(p_hat_logit1)
```
...
```{r}
y_hat_logit2 <- ifelse(p_hat_logit1 > 0.5, "1", "0")
confusionMatrix(data = as.factor(y_hat_logit2), 
                reference = test$underweight1, positive = "1")
```

## Fitting Naive Bayes Model to DHS data

```{r}
nb_fit2  <- naiveBayes(underweight1 ~ anemia2+mother_edu2+wealth_cat+sex, data = train)
p_hat_nb2 <- predict(nb_fit2, test, type = "raw")[,2]
y_hat_nb2 <- predict(nb_fit2, test)

confusionMatrix(data = as.factor(y_hat_nb2), 
                reference = test$underweight1, positive = "1")
```
